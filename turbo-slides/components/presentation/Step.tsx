...
export const Step = ({ children, order, duration = 0.5 }: React.PropsWithChildren & { order: number; duration: number }) => {
  ...
  return (
    <motion.div animate={{ opacity, translateY }} transition={{ duration }} initial={{ opacity: 0, translateY: TRANSLATE_Y_DISTANCE }}>
      {children}
    </motion.div>
  )
}
